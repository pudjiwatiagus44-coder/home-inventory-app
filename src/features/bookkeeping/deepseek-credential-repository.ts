...
export function createPostgresDeepSeekCredentialRepository(
  ...
  return {
    async findForTrustedServerUser(trustedServerUserId) {
      const result = await client.query<CredentialRow>(
        `select ${COLUMNS}
           from bookkeeping_deepseek_credentials
          where user_id = $1
          limit 1`,
        [trustedServerUserId],
      );
    ...
    async saveForTrustedServerUser(trustedServerUserId, credential) {
      const result = await client.query<CredentialRow>(
        `insert into bookkeeping_deepseek_credentials (
      ...
      if (!row) throw new Error("deepseek_credential_user_not_found");
      return normalize(row);
    },

    async recordSuccessfulValidationForTrustedServerUser(
      trustedServerUserId,
      lastVerifiedAt,
    ) {
      const result = await client.query<CredentialRow>(
        `update bookkeeping_deepseek_credentials
    ...
    async deleteForTrustedServerUser(trustedServerUserId) {
      const result = await client.query<{ user_id: string }>(
        `delete from bookkeeping_deepseek_credentials
          where user_id = $1
        returning user_id`,
        [trustedServerUserId],
      );
...
function normalize(row: CredentialRow): StoredDeepSeekCredential {
  return {
