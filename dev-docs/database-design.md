...
- 2026-08-11 区域/位置照片数据设计（`areas.photo_key`、`locations.photo_key`）已写入本文件，migration 已编写并在 `home_inventory_test` 执行成功（`dev-docs/sql/area_location_photos_self_hosted.sql`）。
...
6. 用户审阅区域/位置照片设计后，执行 `dev-docs/sql/area_location_photos_self_hosted.sql` 并同步验收证据；该 migration 已在 `home_inventory_test` 执行成功。
