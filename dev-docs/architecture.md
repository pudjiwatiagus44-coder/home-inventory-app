...
## 2026-08-21 记账 OCR 文本结构化接口

用户确认复用本服务器为 Android 记账 App 提供豆包文本纠错和字段归类能力：

- 新增 `POST /api/bookkeeping/understand`，只接收用户主动截屏后在手机本地生成并脱敏的 OCR 文本；禁止接收、保存或转发屏幕截图。
- 豆包 API key、模型名和基础地址只存服务器环境变量，不写入 APK；当前模型为 `doubao-seed-2-0-mini-260428`。
- 接口返回日期时间、收支类型、类别、金额、币种、付款方/收款方、账户、参与人、标签、商家、项目和备注共 12 个字段，并利用上下文修正常见 OCR 错别字。
- 请求限制文本长度；模型异常、超时或 JSON 不合法时返回明确错误，由 Android 本地解析规则兜底，不能伪造识别成功。
- 当前仅用于本机开发服务器和真机联调；生产部署、鉴权和公网开放状态均为 `未验证`，未经再次确认不部署。

## 2026-08-30 文字批量记账与分类上下文

用户确认将语音记账改为主动触发的文字记账：Android 弹出多行输入框，允许一段文字包含多笔交易，服务端一次返回结构化数组；客户端逐笔校验、保存并通过通知反馈。请求可附带本地账本的父子分类上下文，模型只能从启用分类中选择或返回待确认类别，不能静默新增分类。

分类同步支持父子层级、描述、关键词、图标、颜色、内置标记和启用状态。服务端开发 schema 与接口类型需要同步更新；生产数据库 migration、部署和公网开放仍为 `未验证`，需要单独确认。

## 2026-09-01 记账识别纠错反馈

用户确认继续本地实现识别纠错闭环。服务端新增账号隔离的 `bookkeeping_recognition_feedback` 表和认证反馈接口；Android 每次“保存并报错”都必须先预览并明确授权，只上传脱敏文字、固定 12 字段前后快照、差异项、模型和授权时间。理解接口仅在有效登录会话下检索当前账号最多 3 条相似纠正样本，无会话或查询失败时退化为普通识别。纠正样本作为不可信 JSON 数据，不能改变 system prompt 或输出契约。禁止跨账号共享、自动精调、上传截图和在日志中记录正文。完整边界见 `dev-docs/bookkeeping-recognition-feedback-design.md`；生产迁移和部署仍需再次确认。
