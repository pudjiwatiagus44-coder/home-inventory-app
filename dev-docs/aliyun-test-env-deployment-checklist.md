...
- 标签：未绑定
- 访问方式：测试阶段可先用服务器 IP 访问；正式公开前再绑定已备案域名。
...
Nginx 监听公网 `80`，反向代理到本机 `3000`：

```bash
sudo apt install -y nginx
sudo nano /etc/nginx/sites-available/home-inventory-app
```
...
```bash
sudo ln -s /etc/nginx/sites-available/home-inventory-app /etc/nginx/sites-enabled/home-inventory-app
sudo nginx -t
...
HTTPS 和正式域名证书在 ICP 备案、域名解析和正式公开前再补，不在本测试清单中强行启用。
...
| 80 | TCP | `0.0.0.0/0` | HTTP 测试访问 |
| 443 | TCP | 暂不开放；配置 HTTPS 后再开放 | HTTPS |
| 3000 | TCP | 不对公网开放 | Next.js 本机监听 |
| 5432 | TCP | 不对公网开放 | PostgreSQL 本机访问 |

用户已确认阿里云控制台安全组当前状态与上述测试环境规则一致：`22/tcp` 用于 SSH，`80/tcp` 用于 HTTP 测试访问，`443/tcp` 暂未作为已启用的 HTTPS 入口，`3000/tcp` 和 `5432/tcp` 不对公网开放。
...
- 阿里云账号个人实名认证：用户已确认完成。
- 阿里云控制台安全组：用户已确认当前开放端口与测试环境规则一致。
- Node.js 实际版本：`v24.18.0`。
- PostgreSQL 实际版本：`16.14`。
- Nginx 访问验证：已通过，公网 `http://120.24.93.226/login` 和 `/app` 返回 200。
