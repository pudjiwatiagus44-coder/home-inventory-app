...
| 用户动作 | 打开移动端 `/app` 后优先点击顶部快捷入口，快速搜索物品或新增物品、位置、区域 |
| 可见结果 | 移动端首屏置顶搜索物品、新增物品、新增位置、新增区域入口；点击后弹出对应搜索或新增表单；概览、区域管理、位置管理和物品栏保留在正常页面下方；桌面端仍保持左侧管理、右侧物品清单 |
| 风险等级 | 轻量任务 |
| 当前 owner | `src/features/inventory/AppDashboard.tsx` 负责 dashboard 响应式布局 |
| 推荐插入路线 | 新增移动端快捷弹窗，复用现有 `filters`、`filteredLocations`、`visibleItems` 和新增表单保存逻辑；物品栏在移动端回到下方 |
...
| 第三方服务 | 否 | 不新增服务 |
| 支付/上传/共享 | 否 | 明确不做 |
| UI 基础 | 小幅调整 | 新增移动端搜索/新增快捷弹窗；桌面布局保持不变 |
...
- [x] 移动端 `/app` 顶部提供“搜索物品”入口。
- [x] 移动端点击“搜索物品”后用弹窗搜索名称/备注，并可按区域/位置筛选。
- [x] 移动端 `/app` 顶部提供“新增物品”“新增位置”“新增区域”入口。
- [x] 移动端点击新增入口后用弹窗填写对应表单，保存成功后仍走现有 Supabase/RLS 写入路径。
- [x] 移动端物品栏保留在概览、区域管理和位置管理之后，避免几百个物品把管理入口压到中间。
...
- 手机打开 `/app` 后不需要先滚到物品栏即可打开搜索弹窗。
- 搜索弹窗能显示匹配物品结果，并可关闭回到正常页面。
- 手机打开 `/app` 后不需要滚动即可新增物品、位置或区域。
