...
- 2026-09-09 已完成生产迁移、私有目录、精确反代上限、服务部署与未登录鉴权验证；真实账号上传仍待发布后端到端验收。
...
- SQL 仅新增错误订单报告表、其账户时间索引和附件补偿队列表/claim 索引，不修改库存、认证、同步交易、识别纠错或删除墓碑表。
...
- 真实账号上传、账号隔离负例及后续 AI 分析/人工删除流程尚未执行，继续标记为**未验证**。
