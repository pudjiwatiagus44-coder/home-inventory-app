...
## 2026-09-23 长期登录与登录失效交互验收标准（设计已确认，待实施）

- 会话寿命：超过旧版 30 天期限但未被撤销、账号仍可用的会话继续认证成功；不因固定天数强制用户退出。
- 安全负例：主动退出、密码修改/重置、账号禁用、会话撤销、token 不存在仍返回 401，且不能访问或修改库存数据。
- Web 续期：有效的已认证活动滚动刷新 HttpOnly + Secure + SameSite=Lax Cookie；无效会话不续期并进入登录页。
- Android 失效处理：新增物品、刷新清单或其他受保护请求返回 401 后，清除旧 Cookie、自动返回登录页并显示“登录已失效，请重新登录”。
- Web 失效处理：受保护请求返回 401 后跳转 `/login` 并显示相同中文提示。
- 文案：Web/Android 用户界面不再直接显示 `Authentication required`。
- 数据保护：401 处理不得删除 Android 本地库存缓存或草稿；重新登录后可继续新增物品。
- 验证：服务端单测覆盖长期/撤销/密码重置会话和 Cookie 续期；Android 与 Web 测试覆盖 401 状态切换；本地全量回归通过后才允许申请生产迁移和部署授权。
...
## 2026-09-01 记账识别纠错反馈设计确认

- 用户确认继续本地实现，生产数据库迁移和部署另行确认。
