...
| 用户动作 | 打开移动端 `/app` 后优先点击顶部“搜索物品”，在弹窗里搜索和筛选，而不是先滚到物品栏 |
| 可见结果 | 移动端首屏只置顶搜索入口；点击后弹出搜索框、区域/位置筛选和匹配物品结果；概览、区域管理、位置管理和物品栏保留在正常页面下方；桌面端仍保持左侧管理、右侧物品清单 |
| 风险等级 | 轻量任务 |
| 当前 owner | `src/features/inventory/AppDashboard.tsx` 负责 dashboard 响应式布局 |
| 推荐插入路线 | 新增移动端搜索弹窗，复用现有 `filters`、`filteredLocations` 和 `visibleItems`；物品栏在移动端回到下方 |
| 拒绝路线 | 不新增数据库字段；不修改 RLS；不改变物品/区域/位置业务逻辑；不新增独立移动端路由；不把新增物品放进搜索弹窗 |
