...
- `/app` 当前能读取 household、areas、items 概览。
- “新增物品”按钮当前为禁用状态。
- 用户 A/B 权限负例尚未完成验证。
...
- 未验证：真实 Supabase 项目中的用户 A/B RLS 负例。
...
- [ ] 确认现有 dashboard 数据类型覆盖 household、locations、items。
- [ ] 补齐单元测试，覆盖空数据、已有物品、缺失 household 名称。
- [ ] 保持未登录访问 `/app` 时跳转或提示登录。
- [ ] 验证未配置 Supabase env 时不假装成功。
- [ ] 不新增 mock 数据。
...
- [ ] 为区域提供删除入口，删除前必须确认。
- [ ] 在 `/app` 中提供新增位置入口。
- [ ] 区域表单字段包含 `name` 和 `color`。
- [ ] 位置表单字段包含 `name` 和可选 `area_id`。
- [ ] 写入 `areas` 时必须带当前用户自己的 `household_id`。
- [ ] 写入 `locations` 时必须带当前用户自己的 `household_id`。
- [ ] `area_id` 只能来自当前 household 的区域，允许不选择区域。
- [ ] 保存成功后刷新区域和位置列表。
- [ ] 删除区域后，该区域下的位置显示为“未分区”，位置和物品不应被删除。
- [ ] 保存失败时显示明确错误。
...
- [ ] 启用“新增物品”入口。
- [ ] 表单字段包含 `name`、`location_id`、`note`、`expire_date`。
- [ ] `name` 必填，并遵守数据库长度限制。
- [ ] `location_id` 只能来自当前 household 的位置。
- [ ] 保存成功后出现在物品列表。
...
- [ ] 列表展示物品名称、区域、位置、备注摘要、过期日期。
- [ ] 提供基础搜索输入。
- [ ] 搜索范围为 `items.name` 和 `items.note`。
- [ ] 提供区域筛选。
- [ ] 提供位置筛选。
- [ ] 空搜索结果显示清晰提示。
- [ ] 不引入全文搜索索引；MVP 使用 `ilike` 即可。
...
- [ ] 准备用户 A 和用户 B。
- [ ] 用户 A 创建 area、location 和 item。
- [ ] 用户 B 登录后无法看到用户 A 的数据。
- [ ] 用户 B 使用用户 A 的 `household_id` 插入 area 失败。
- [ ] 用户 B 使用用户 A 的 `household_id` 插入 item 失败。
- [ ] 用户 B 使用用户 A 的 `item_id` 更新 item 失败。
- [ ] 用户 B 使用用户 A 的 `item_id` 删除 item 失败。
- [ ] 未登录请求不能读取用户数据。
...
- 真实 Supabase 项目中的用户 A/B RLS 负例。
- 默认 household 初始化函数在真实 Supabase 项目中的完整行为。
- 新增位置和新增物品的真实浏览器路径。
- 搜索、编辑、删除的真实浏览器路径。
- 移动端完整操作体验。
- 阶段完成后的 Git checkpoint。
