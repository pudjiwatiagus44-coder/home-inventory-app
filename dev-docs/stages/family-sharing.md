...
- 2026-08-06 Android 内测版邀请分享已实现：App 顶栏“邀请”按钮生成链接，可复制或系统分享；APK 0.2.0 已托管到服务器并接入落地页下载入口（自动上传脚本 `scripts/upload-apk.ps1`）。
- 2026-08-06 Android 内测版申请审批已实现：邀请弹窗内展示待处理加入申请，房主可直接批准/拒绝；APK 0.3.0 已托管。
- 2026-08-06 Android 更新提醒已实现：进入清单页时检查 `/apk/version.json`，服务器版本更新时弹窗提示下载；APK 0.4.0 已托管。
- 真实浏览器验收陪跑（房主生成链接 → 家人申请 → 批准 → 共同编辑 → 移除成员）。
- Android 真机点击验收（邀请 → 分享 → 申请 → App 批准 → 更新提醒）。
...
- 多家庭数据合并或迁移。
- Android 端的申请审批/成员管理 UI（本轮只做房主邀请分享）。
