...
- 当前结论：中国大陆正式版需要新部署路线；Vercel + Supabase 仅保留为临时测试版，不作为中国大陆正式生产方案。
...
- 主推荐云平台：阿里云或腾讯云二选一，优先选择同一个平台完成域名、备案、服务器、数据库和日志。
...
- 域名：购买可备案域名，并确认实名信息。
- 云账号：选择阿里云或腾讯云，并完成实名认证。
- 服务器/托管资源：购买符合 ICP 备案要求的中国大陆地域资源。
- ICP 备案：通过云服务商备案系统提交主体、网站和服务器接入信息。
